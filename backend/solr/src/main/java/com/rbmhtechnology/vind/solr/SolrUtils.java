...
import com.rbmhtechnology.vind.solr.SolrUtils.Fieldname.UseCase;
...
public class SolrUtils {
    ...
    public static final class Query {
        ...
        public static String serializeFilter(Filter filter, DocumentFactory factory,String searchContext, boolean strict) {
            ...
            return serializedFilters.equals("")? typeFilterString : String.join(" +", typeFilterString, "("+serializedFilters+")");
        }
            ...
            final Optional<String> facetOptional = search.getFacets().values().stream()
                    ...
                    .map(facet -> {
                        ...
                        if(Objects.nonNull(search.getChildrenSearchString().hasFilter())){
                          ...
                          filter = serializeFilter(search.getChildrenSearchString().getFilter(), search.getChildrenFactory(), searchContext, search.getStrict()).replaceAll("\\+_type_:" + type + " \\+","").replaceAll("\"", "")+ " AND " + search.getSearchString();
                        } else {
                        ...
                        return String.format(
                                "{parent_facet:{" +
                                        ...
                                        "}" +
                                        "}" +
                                        "}}", Fieldname.ID, Fieldname.TYPE, type, filter, Fieldname.TYPE, type);
                    }).findAny();
    ...
    public static final class Result {
        ...
        public static FacetResults buildFacetResult(QueryResponse response, DocumentFactory factory, Map<String,Facet>  facetsQuery, String searchContext) {
            ...
            return new FacetResults(factory, facets, typeFacetResults, queryFacetResults, rangeFacetResults, intervalFacetResults, statsResults, pivotFacetResults);
        }
