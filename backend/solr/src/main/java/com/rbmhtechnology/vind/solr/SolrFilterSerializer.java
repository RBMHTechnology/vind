...
public class SolrFilterSerializer {
    ...
    public String serialize(Filter filter, String searchContext) {
        //TODO: Add the integration with the nonStrict search flag
        if(!strict) {
    ...
    public String serialize(Filter.WithinBBoxFilter filter, String searchContext) {
        ...
        return String.format("%s:[%s TO %s]", getFieldName(filter.getField(), searchContext,UseCase.valueOf(filter.getFilterScope(filter.getField(),factory).toString())), filter.getUpperLeft(), filter.getLowerRight());
    }
    ...
    public String serialize(Filter.ChildrenDocumentFilter filter, String searchContext) {
        if(StringUtils.isNotEmpty(filter.getNestedDocType())) {
            return String.format("{!parent which=\"%s:%s\"} (%s:* AND %s:%s)", TYPE, filter.getParentDocType(), ID, TYPE, filter.getNestedDocType());
        } else {
            return String.format("{!parent which=\"%s:%s\"} (%s:* AND -%s:%s)", TYPE, filter.getParentDocType(), ID, TYPE, filter.getParentDocType());
        }
    }
