package com.rbmhtechnology.vind.solr;
...
import static com.rbmhtechnology.vind.solr.SolrUtils.Fieldname.*;
import static com.rbmhtechnology.vind.solr.SolrUtils.Query.buildFilterString;
...
public class SolrSearchServer extends SearchServer {
    ...
    protected SolrQuery buildSolrQuery(FulltextSearch search, DocumentFactory factory) {
        ...
        //filters
        if(search.hasFilter()) {
            buildFilterString(search.getFilter(), factory,search.getChildrenFactory(),query, searchContext, search.getStrict());
        } else {
        ...
        if(search.isChildrenSearchEnabled()) {
            ...
            if(search.getChildrenSearchString().hasFilter()){
                ...
                //TODO clean up!
                final String parentFilterQuery =  "(+" + String.join(" +", query.getFilterQueries()) + ")";
        ...
        if(search.hasFacet()) {
            ...
            search.getFacets().values().stream()
                    ...
                    .forEach(queryFacet ->
                           query.addFacetQuery(StringUtils.join(SolrUtils.Query.buildSolrFacetCustomName(buildFilterString(queryFacet.getFilter(), factory, search.getChildrenFactory(), searchContext,search.getStrict()), queryFacet)))
                    );
            //Numeric Range facet
            search.getFacets().values().stream()
                                ...
                                .stream().forEach(context -> {
                                    ...
                                    if (fieldName != null) {
                                        ...
                                        updateOptions.get(fieldDescriptor).get(context).stream().forEach(entry -> {
                                            ...
                                            if(fieldName.startsWith("dynamic_single_") && useCase.equals(UseCase.Sort) && opType.equals(Update.UpdateOperations.add)) {
    ...
    public void execute(Delete delete, DocumentFactory factory) {
        String query = buildFilterString(delete.getQuery(), factory, delete.getUpdateContext(),true);
        try {
    ...
    public SuggestionResult execute(ExecutableSuggestionSearch search, DocumentFactory assets) {
        ...
        try {
            ...
            if(response!=null){
                return SolrUtils.Result.buildSuggestionResult(response, assets,search.getSearchContext());
            }else {
    ...
    protected SolrQuery buildSolrQuery(ExecutableSuggestionSearch search, DocumentFactory assets) {
        final String searchContext = search.getSearchContext();
        ...
        if(search.isStringSuggestion()) {
            ...
            query.setParam("suggestion.field", s.getSuggestionFields()
                    .stream()
                    .map(name ->
                            getFieldname(assets.getField(name), UseCase.Suggest, searchContext))
        ...
        //filters
        if(search.hasFilter()) {
            buildFilterString(search.getFilter(), assets,query, searchContext, true);
        }
        return query;
    }
